...
import { addbookService, bookService,  deletebookService,  getbookService, updatebookService } from "../Service/book.service";
import {Book} from '../drizzle/book'
...
export const addBook = async (c: Context) => {
    try {
        ...
        const addedBook = await addbookService(book);


        if (!addedBook) return c.text("Book not created", 404);
        return c.json({ msg: addedBook }, 201);
